...
class DLA(nn.Module):
    ...
    def load_pretrained_model(self, data='imagenet', name='dla102-d94d9790.pth', hash='d94d9790'):
        fc = self.fc
        if name.endswith('.pth'):
            print('load from : ', name)
            model_weights = torch.load('./checkpoint/'+name)
        else:
...
def dla34(pretrained=False, pretrained_model_path=None, **kwargs):  # DLA-34
    ...
    if pretrained:
        if not pretrained_model_path:
            ...
            model.load_pretrained_model(data='imagenet', name=pretrained_model_path, hash='ba72cf86')
...
def dla102(pretrained=False, **kwargs):  # DLA-102
    ...
    if pretrained:
        model.load_pretrained_model(pretrained, 'dla102-d94d9790.pth')
    return model
