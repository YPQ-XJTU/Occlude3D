...
class Center_guidedTransformer(nn.Module):
    def __init__(
            self,
            d_model=256,
            nhead=8,
        ...
        decoder_layer = Center_guidedDecoderLayer(
            d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, dec_n_points, group_num=group_num)
        self.decoder = Center_guidedDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec, d_model, use_dab=use_dab, two_stage_dino=two_stage_dino)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, depth_pos_embed=None, depth_pos_embed_ip=None, attn_mask=None):
        ...
        if self.two_stage:#False
            ...
            tgt = tgt.unsqueeze(0).expand(bs, -1, -1)
            #print(query_embed.size(),tgt.size())
...
class VisualEncoderLayer(nn.Module):
    ...
    def with_pos_embed(tensor, pos):
        #如果有pos信息的话，把位置编码与输入张量相加，以提供位置信息
        # print('pos size: ',pos.size())#[16, 10200, 256]
        # print('tensor size: ',tensor.size())#[16, 10200, 256]
...
class Center_guidedDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        # print(tgt2.size())
        txt = tgt2.view(-1, tgt2.size(2))
        np.savetxt('./outputs/tensor.txt',txt.cpu().numpy())
...
class Center_guidedDecoder(nn.Module):
...
def build_center_guided_transformer(cfg):
    return Center_guidedTransformer(
