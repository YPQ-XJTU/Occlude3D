"""
Occluded3d: Make occlusion clear for Monocular 3D Object Detection
"""
import torch
...
from .center_guided_transformer import build_center_guided_transformer
from lib.losses.focal_loss import sigmoid_focal_loss
from ddn_loss import DDNLoss
from Modules import CenterFusionModule , DepthEncoder

def _get_clones(module, N):
    return nn.ModuleList([copy.deepcopy(module) for i in range(N)])


class Occlude3D(nn.Module):
    def __init__(self, backbone, center_guided_transformer,num_classes, num_queries, num_feature_levels,
        ...
        Parameters:
            ...
            center_guided_transformer: transformer architecture. See center_guided_transformer.py
        ...
        self.center_guided_transformer = center_guided_transformer
        self.center_fusion = CenterFusionModule
        self.depth_encoder = DepthEncoder
        hidden_dim = center_guided_transformer.d_model
        ...
        if not two_stage:
            ...
            if not use_dab:
                ...
                #print('query_embed size: ',self.query_embed.weight.size())
            else:
                self.tgt_embed = nn.Embedding(num_queries * group_num, hidden_dim)
                self.refpoint_embed = nn.Embedding(num_queries * group_num, 6)

        if num_feature_levels > 1:
            #如果特征层级>1，则表明模型需要处理多个特征层，需要在每一层都设置特征投影
            num_backbone_outs = len(backbone.strides)
            input_proj_list = []
            for _ in range(num_backbone_outs):
                #根据backbone的输出层级书来判断需要几个特征投影层
                in_channels = backbone.num_channels[_]
                #每层投影都是通过一个1*1的卷积层和一个GroupNorm归一化层来将输入特征调整为hidden_dim，保证了不同层级的特征具有相同的维度
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
                    nn.GroupNorm(32, hidden_dim),
                ))
            #如果存在额外的层级，就对多出来的层级设置特征投影层，设置为3*3的卷积层，步幅2，来降低特征的空间尺寸，保持
            #这样做是为了处理来自更低分辨率的特征，以提高模型的感受野和特征的多尺度表达能力
            ...
            #最后将构建好的input_proj组成一个nn.ModuleList进行封装
        ...
        if self.two_stage_dino:        
            ...
            self.center_guided_transformer.enc_out_bbox_embed = copy.deepcopy(_bbox_embed)
            self.center_guided_transformer.enc_out_class_embed = copy.deepcopy(_class_embed)

        for proj in self.input_proj:
            #对之前定义好的input_proj再次进行参数归零，初始化
            nn.init.xavier_uniform_(proj[0].weight, gain=1)
            nn.init.constant_(proj[0].bias, 0)


        num_pred = (center_guided_transformer.decoder.num_layers + 1) if two_stage else center_guided_transformer.decoder.num_layers

        if with_box_refine:
            self.class_embed = _get_clones(self.class_embed, num_pred)
            self.bbox_embed = _get_clones(self.bbox_embed, num_pred)
            #对边界框嵌入器进行初始化，将其最后一层的偏置参数的一部分设置为固定值-2.0，这是一种针对迭代边界框细化的实现技巧
            nn.init.constant_(self.bbox_embed[0].layers[-1].bias.data[2:], -2.0)
            # hack implementation for iterative bounding box refinement
            self.center_guided_transformer.decoder.bbox_embed = self.bbox_embed
            self.dim_embed_3d = _get_clones(self.dim_embed_3d, num_pred)
            self.center_guided_transformer.decoder.dim_embed = self.dim_embed_3d
            ...
            self.center_guided_transformer.decoder.bbox_embed = None

        if two_stage:
            self.center_guided_transformer.decoder.class_embed = self.class_embed
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    #在第一个需要新增的特征层次中，直接从骨干网络中获取最后一层特征，并进行投影处理。
                    src = self.input_proj[l](features[-1].tensors)
                else:
                    #对上一层的特征进行采样获取当前层的特征
                    src = self.input_proj[l](srcs[-1])
                #创建了一个全零的掩码张量，用于指示要进行插值的区域。
                m = torch.zeros(src.shape[0], src.shape[2], src.shape[3]).to(torch.bool).to(src.device)
                #对掩码进行操作，使其与当前特征的大小匹配，并转化为bool格式
                mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]
                #利用backbone的位置编码器来计算当前特征的位置编码
        ...
        fusion_feature = self.center_fusion(srcs, masks, pos)
        vis_depth_embed , att_depth_embed = self.depth_encoder(vis_depth,att_depth)

        #[50, 512],[16, 256, 24, 80],[16, 256, 24, 80]

        hs, init_reference, inter_references, inter_references_dim, enc_outputs_class, enc_outputs_coord_unact = self.center_guided_transformer(
            fusion_feature, query_embeds, vis_depth_embed,att_depth_embed )
...
def build(cfg):
    ...
    # detr
    center_guided_transformer= build_center_guided_transformer(cfg)


    model = Occlude3D(
        backbone,
        center_guided_transformer,
